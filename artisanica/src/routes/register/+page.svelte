...
<div class="flex h-screen w-full flex-row items-center justify-center bg-[#F9FAFB]">
	<div class=" h-ful flex w-full flex-col items-center justify-center gap-4">
		<form
			...
			action="?/register"
		>
			<h1 class="text-center text-3xl text-[#1F2937]">Create an account</h1>
			<input
				type="text"
			...
			<button class="rounded bg-[#1F2937] px-4 py-2 text-[#fff]">Register</button>
		</form>
	</div>
	{#if form?.message}
		<div class="absolute top-5 flex bg-[#1F2937] p-4 text-[#fff]">
			<p>Registration failed, try again later</p>
		</div>
	{/if}
</div>
